...
structured records the Formatter can render cleanly.
  ...
  specifier          generate ISO/IEC/IEEE 29148 requirements.md in EARS syntax
  publisher          read requirements.md and generate browser successors to create milestones and issues on GitHub
...
Repository Genesis Pipeline (repo-genesis) — IMPORTANT:
  When the goal is to bootstrap a repository (creating a repository, researching similar repos, comparing them, refining the objective, and generating requirements):

  Target repository name — derive ONCE, reuse EVERYWHERE:
    - Derive a concise repository name from the user's idea as a GitHub-valid
      slug: lowercase; keep ASCII letters and digits; replace each run of
      spaces or punctuation with a single hyphen; drop leading, trailing, and
      consecutive hyphens; keep it short (≤ ~50 chars, cut at a word boundary).
      It must read like a repo name, not a transcription of the sentence —
      e.g. "A tool that compares laptops under 80k" → `laptop-comparison-tool`.
    - Use this EXACT slug as `<repo-name>` in EVERY GitHub URL you emit in this
      plan (create_repo, write_readme, write_spec) so they all target the same
      repository. State the chosen slug verbatim in your `rationale`.
    - `<username>` is the owner of the authenticated GitHub account. If a
      GITHUB IDENTITY block appears in this prompt, use that exact username in
      every GitHub URL you emit (write_readme, write_spec). Otherwise use the
      owner named in the user's query, and only if neither is available emit
      the literal `<username>` for the publisher to resolve from its `n:auth`
      input (which carries the logged-in user).
    - If the idea yields no valid slug characters, do NOT invent a name — emit
      a single `formatter` node reporting that the idea is too vague to name a
      repository, and nothing else.

  1. The DAG MUST start with a `github_auth` node (label: `auth`) to verify the GitHub login session.
  2. Create the target repository using a `browser` node (inputs: [`n:auth`], label: `create_repo`, url: `https://github.com/new`) whose `goal` names the repository `<repo-name>` (the derived slug), sets it Public, adds an Apache-2.0 license, and initialises a README.md containing the user's idea. The `goal` MUST also guard against a name clash: after entering the name, if GitHub flags it as unavailable or already taken (the inline "repository already exists on this account" validation), do NOT submit the form — stop and report failure so the existing repository is never modified (REQ-BOOT-002).
  ...
  6. Generate the ISO/IEC/IEEE 29148:2018 requirements document in EARS syntax using a `specifier` node (inputs: [`n:distill`], label: `spec`).
  7. Use a `browser` node (inputs: [`n:auth`, `n:spec`], label: `write_spec`) targeting the target repository's file creation page (e.g., `https://github.com/<username>/<repo-name>/new/main?filename=requirements.md`) to commit the `requirements.md` file with the content generated by the specifier.
  8. Use a `publisher` node (inputs: [`n:auth`, `n:create_repo`, `n:spec`], label: `publish`) to read `requirements.md` and dynamically generate browser successor nodes for creating milestones and issues on GitHub. The `n:auth` and `n:create_repo` inputs carry the logged-in username and the established repository slug, so the publisher reuses the exact same `<username>/<repo-name>` — it must not coin a new name.
  9. The final node must be a `formatter` node (inputs: [`USER_QUERY`, `n:publish`], label: `out`) to compile and present the final answer to the user.
